...
def fit(epochs, model, loss_func, opt, train_dl, valid_dl, one_cycle=None, train_metric=False):
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            val_loss, val_accuracy, (y_true, predictions) = validate(model, valid_dl, loss_func)
            if train_metric:
                train_loss, train_accuracy, (y_true, predictions) = validate(model, train_dl, loss_func)
            else:
    ...
    return model, metrics, (y_true, predictions)
